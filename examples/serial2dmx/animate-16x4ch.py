...
if len(sys.argv) >= 2:
    ...
    print
    print "Usage:", sys.argv[0], " AUTOMATION [4CH-MASK] [SPEED] [DURATION]"
    print
    print "AUTOMATION  pulse | sweep | peews | constant"
    print "4CH-MASK    0..255,0..255,0..255,0..255"
    print "SPEED       1..1000"
    print "DURATION    seconds per frame (float)"
    print
...
def pulse():
    ...
    while True:
        factor += step
        if factor < 0 or factor > maxFactor:
            step *= -1
            factor = coerceInt( factor, 0, maxFactor )
...
def constant():
    while True:
        send( quadValueFromFactor( maxFactor ) * activeQuadChannels )
...
def printMode(mode): print mode, "on", "4x"+str(activeQuadChannels), "channels with mask", colourCSV
